...
bool downloadFileToLittleFS(const String& url, const String& filename) {
...
void handleRoot(AsyncWebServerRequest *request) {
  ...
  response->print("else if(d.type==='download_status'){alert(d.msg)}");
  ...
  response->print("ws.send(JSON.stringify({action:'download_files'}));alert('Download started...')}}");
  ...
  if (type == WS_EVT_CONNECT) {
    ...
    if (info->final && info->index == 0 && info->len == len && info->opcode == WS_TEXT) {
      ...
      if (action == "get_config") {
        ...
        statusDoc["type"] = "download_status";
        if (timer_ok && ace_ok) {
          statusDoc["msg"] = "Files downloaded successfully!";
        } else if (timer_ok) {
          statusDoc["msg"] = "timer.html downloaded, but ace.html failed";
        } else if (ace_ok) {
          statusDoc["msg"] = "ace.html downloaded, but timer.html failed";
        } else {
          statusDoc["msg"] = "Download failed for both files";
        }
