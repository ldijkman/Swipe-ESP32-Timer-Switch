...
// WiFi credentials
const char* ssid = "wifibroadcasted";
...
// Authentication credentials - Change these to your preferred username/password
...
void setup() {
  ...
  server.on("/settings", HTTP_POST, 
    ...
    [](AsyncWebServerRequest *request, uint8_t *data, size_t len, size_t index, size_t total) {
      handleSettings(request, data, len, index, total);
    }
  );
  ...
  // Camera stream proxy - forward stream from camera device
  server.on("/camera-stream", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    if (!request->hasParam("ip")) {
      request->send(400, "text/plain", "Missing camera IP parameter");
      return;
    }
    
    String cameraIP = request->getParam("ip")->value();
    Serial.print("Proxying camera stream from: ");
    Serial.println(cameraIP);
    Serial.print("Free heap: ");
    Serial.println(ESP.getFreeHeap());
    
    // Create WiFi client to connect to camera
    WiFiClient* client = new WiFiClient();
    uint32_t* lastActivity = new uint32_t(millis());
    bool* active = new bool(true);
    uint32_t* frameCount = new uint32_t(0);
    
    if (!client->connect(cameraIP.c_str(), 81, 3000)) {
    ...
    while (client->available() == 0) {
      if (millis() - timeout > 8000) {
        ...
        request->send(504, "text/plain", "Camera timeout");
        client->stop();
        delete client;
        delete lastActivity;
        delete active;
    ...
    String contentType = "multipart/x-mixed-replace; boundary=frame";
    ...
    Serial.print("Streaming with content type: ");
    Serial.println(contentType);
    
    // Create chunked response that streams camera data
    AsyncWebServerResponse *response = request->beginChunkedResponse(contentType.c_str(),
      [client, lastActivity, active, frameCount](uint8_t *buffer, size_t maxLen, size_t index) -> size_t {
        ...
        if (!client->connected() && !client->available()) {
          Serial.print("Camera disconnected. Frames: ");
        ...
        // Wait if no data available - GIVE TIME TO OTHER TASKS
        if (!client->available()) {
          vTaskDelay(1); // Give 1 tick to other tasks (including async_tcp)
          return RESPONSE_TRY_AGAIN;
        }
        
        // Read VERY SMALL chunks to return control frequently
        size_t available = client->available();
        size_t toRead = min(available, min(maxLen, (size_t)512)); // Only 512 bytes at a time
        size_t bytesRead = 0;
        
        // Read the chunk
        while (bytesRead < toRead && client->available()) {
        ...
        // Give time slice to other tasks before returning
        vTaskDelay(1);
  ...
  // Camera stream proxy with frame rate limiting (reduced load)
  server.on("/camera-stream", HTTP_GET, [](AsyncWebServerRequest *request) {
    if (!request->authenticate(www_username, www_password)) {
      return request->requestAuthentication();
    }
    ...
    // Get FPS parameter (default 5 fps, max 10 fps)
    int targetFPS = 5;
    if (request->hasParam("fps")) {
      targetFPS = request->getParam("fps")->value().toInt();
      targetFPS = constrain(targetFPS, 1, 10);
    }
    ...
    // Create WiFi client
    WiFiClient* client = new WiFiClient();
    uint32_t* lastFrameTime = new uint32_t(0);
    uint32_t* frameInterval = new uint32_t(1000 / targetFPS); // ms per frame
    bool* active = new bool(true);
    uint32_t* frameCount = new uint32_t(0);
    uint32_t* lastActivity = new uint32_t(millis());
    ...
    Serial.print("Stream started with content type: ");
    Serial.println(contentType);
    
    // Create response with frame rate limiting
    AsyncWebServerResponse *response = request->beginChunkedResponse(contentType.c_str(),
      [client, lastFrameTime, frameInterval, active, frameCount, lastActivity](uint8_t *buffer, size_t maxLen, size_t index) -> size_t {
        
        // Check if should end
        if (!(*active)) {
          Serial.print("Stream ended. Frames sent: ");
          Serial.println(*frameCount);
          if (client->connected()) client->stop();
          delete client;
          delete lastFrameTime;
        ...
        if (now - (*lastFrameTime) < (*frameInterval)) {
          // Too soon, discard this data and wait
          while (client->available()) {
            client->read(); // Discard data
          }
          vTaskDelay(pdMS_TO_TICKS(10));
          return RESPONSE_TRY_AGAIN;
        }
        ...
        *lastActivity = now;
        ...
        // Read frame data in small chunks
        size_t available = client->available();
        size_t toRead = min(available, min(maxLen, (size_t)1024));
        size_t bytesRead = 0;
        
        while (bytesRead < toRead && client->available()) {
          buffer[bytesRead++] = client->read();
          if (bytesRead % 256 == 0) {
            vTaskDelay(1); // Yield every 256 bytes
          }
        }
  ...
  // Note: Camera stream proxy removed - ESP32 doesn't have enough resources
  // Users should forward camera port 81 separately for internet access
    ...
    if (jsonStr.length() > 0) {
      ...
      if (!deserializeJson(doc, jsonStr)) {
        if (doc["DeviceName"]) {
          ...
          String devName = devices[index].deviceName;
          devName.toLowerCase();
          if (devName.indexOf("camera") >= 0 || 
              devName.indexOf("cam") >= 0 ||
              devName.indexOf("esp32-cam") >= 0 ||
              devName.indexOf("ai-thinker") >= 0) {
            devices[index].isCamera = true;
            Serial.println("    Detected as CAMERA device");
          }
        }
      }
    }
...
void sendDeviceList() {
  if (xSemaphoreTake(deviceMutex, portMAX_DELAY)) {
    ...
    for (int i = 0; i < deviceCount; i++) {
      ...
      device["isCamera"] = devices[i].isCamera;
...
<script>
    ...
    function updateDeviceGrid() {
      ...
      grid.innerHTML = devices.map(function(device) {
        ...
        if (device.isCamera) {
          return `
            <div class="device" style="position: relative; border: 2px solid #17a2b8;">
              ...
              <div style="margin: 10px 0; display: grid; gap: 6px;">
                <a href="http://${device.ip}:81/stream" target="_blank" style="display: block; padding: 10px; background: #28a745; color: white; text-align: center; border-radius: 5px; text-decoration: none; font-weight: bold;">
                  📷 DIRECT STREAM (Full Speed)
                </a>
                <a href="/camera-stream?ip=${device.ip}&fps=5" target="_blank" style="display: block; padding: 8px; background: #17a2b8; color: white; text-align: center; border-radius: 5px; text-decoration: none; font-size: 14px;">
                  🌐 PROXIED (5 FPS)
                </a>
                <a href="/camera-stream?ip=${device.ip}&fps=2" target="_blank" style="display: block; padding: 8px; background: #6c757d; color: white; text-align: center; border-radius: 5px; text-decoration: none; font-size: 13px;">
                  🌐 PROXIED (2 FPS - Slower)
                </a>
              </div>
              <div style="font-size: 10px; color: #333; background: #e7f3ff; padding: 8px; border-radius: 4px; margin-top: 6px; border-left: 3px solid #17a2b8;">
                <strong>💡 TIP:</strong> Proxied streams work from internet but are limited FPS to prevent ESP32 overload
              </div>
              <div style="font-size: 11px; color: #333; background: #fff3cd; padding: 10px; border-radius: 4px; margin-top: 6px; border-left: 3px solid #ffc107;">
                <strong>🌐 Best for Internet:</strong><br>
                Forward port 81 → 8181 on routers for full speed access
              </div>
        ...
        // Power monitoring section (only if device has it)
        let powerSection = '';
