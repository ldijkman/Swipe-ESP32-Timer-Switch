...
void setup() {
  ...
  server.on("/camera-stream", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    // Wait for response headers with longer timeout
    unsigned long timeout = millis();
    while (client->available() == 0) {
      if (millis() - timeout > 10000) {
        Serial.println("Camera timeout waiting for response");
    ...
    Serial.println("Got response, reading headers...");
    ...
    Serial.print("Starting stream with content type: ");
    Serial.println(contentType);
    
    // Create chunked response
    AsyncWebServerResponse *response = request->beginChunkedResponse(
      contentType.c_str(),
      [client, lastActivity, active, totalBytes](uint8_t *buffer, size_t maxLen, size_t index) -> size_t {
        ...
        if (!(*active)) {
          Serial.print("Stream ended. Total bytes: ");
        ...
        // Timeout check - stop after 60 seconds of no data
        if (millis() - (*lastActivity) > 60000) {
          Serial.println("Stream timeout - no data for 60s");
          *active = false;
          if (client->connected()) client->stop();
          delete client;
          delete lastActivity;
        ...
        // Read data in smaller chunks and feed watchdog
        size_t available = client->available();
        size_t toRead = min(available, min(maxLen, (size_t)2048)); // Reduced from 4KB to 2KB
        size_t bytesRead = 0;
        
        while (bytesRead < toRead && client->available()) {
          buffer[bytesRead++] = client->read();
          
          // Feed watchdog every 512 bytes
          if (bytesRead % 512 == 0) {
            yield();
          }
        }
        ...
        yield(); // Feed watchdog before returning
        return bytesRead;
      }
    );
    
    request->send(response);
    Serial.println("Stream response sent to client");
  });
  
  // Camera view page (simpler alternative using img tag)
  server.on("/camera-view", HTTP_GET, [](AsyncWebServerRequest *request) {
    if (!request->authenticate(www_username, www_password)) {
      return request->requestAuthentication();
    }
...
<body>
  ...
  <div class="stream-container">
    <img src="/camera-stream?ip=)rawliteral" + cameraIP + R"rawliteral(" alt="Camera Stream">
  </div>
</body>
</html>
)rawliteral";
          ...
          return `
            <div class="device" style="position: relative; border: 2px solid #17a2b8;">
              ...
              <div style="margin: 10px 0; display: grid; gap: 8px;">
                <a href="/camera-view?ip=${device.ip}" target="_blank" style="display: block; padding: 10px; background: #17a2b8; color: white; text-align: center; border-radius: 5px; text-decoration: none; font-weight: bold;">
                  📷 VIEW STREAM (Proxied)
                </a>
                <a href="http://${device.ip}:81/stream" target="_blank" style="display: block; padding: 8px; background: #6c757d; color: white; text-align: center; border-radius: 5px; text-decoration: none; font-size: 13px;">
                  🔗 Direct Stream (Local Only)
                </a>
              </div>
